...
let pagesMeta: Record<string, PageMeta> = {};
try {
...
if (siteSettings.lang && siteSettings.lang !== "en") {
  indexTemplate = indexTemplate.replace(
    '<html lang="en">',
    `<html lang="${escapeHtml(siteSettings.lang)}">`,
  );
}
if (siteSettings.favicon_url) {
  indexTemplate = indexTemplate.replace(
    '<link rel="icon" type="image/svg+xml" href="/favicon.svg" />',
    `<link rel="icon" href="${escapeHtml(siteSettings.favicon_url)}" />`,
  );
}
...
if (Object.keys(clientSettings).length > 0) {
  indexTemplate = indexTemplate.replace(
    "</head>",
    `<script>window.__ZO_SITE_SETTINGS__=${JSON.stringify(clientSettings).replace(/</g, "\\u003c")}</script>\n</head>`,
  );
}
...
function injectMeta(rawPath: string): string {
  ...
  if (pageMeta?.page_noindex === "block") noindex = true;
  else if (pageMeta?.page_noindex === "allow") noindex = false;
  if (noindex) {
    html = html.replace(
      "</head>",
...
app.use(async (c, next) => {
  ...
  if (
    /\.(png|jpe?g|gif|webp|svg|ico|woff2?|ttf|eot|mp[34]|webm|pdf)$/i.test(path)
  ) {
    return c.notFound();
  }
